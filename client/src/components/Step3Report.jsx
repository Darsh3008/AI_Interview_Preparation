...
const Step3Report = ({ report }) => {
  ...
  const questionsScoreData = questionWiseScore.map((item, index) => ({
    name: `Q${index + 1}`,
    score: item.score ?? item.finalScore ?? item.rating ?? 0,
  }));
  const avgConfidence =
    questionWiseScore.length > 0
      ? questionWiseScore.reduce(
  ...
  const skills = [
    {
      label: "Confidence",
      value: Number(avgConfidence.toFixed(1)),
    },
    {
      label: "Communication",
      value: Number(avgCommunication.toFixed(1)),
    },
    {
      label: "Correctness",
      value: Number(avgCorrectness.toFixed(1)),
    },
  ];
  ...
  let ShortTagLine = "";
  if (finalScore >= 8) {
    perfromanceText = "Ready for job opportunities.";
    ShortTagLine = "Excellent clarity and structured responses."
  }
  else if (finalScore >= 5) {
    perfromanceText = "Needs minor improvement before interviews.";
    ShortTagLine = "Good Foundation, refine articulations."
  }
  else {
    perfromanceText = "Significant improvement required.";
  ...
  const download = () => {
    ...
    doc.text(`Confidence : ${avgConfidence}`, margin + 10, currentY + 10);
    doc.text(`Communication : ${avgCommunication}`, margin + 10, currentY + 18);
    doc.text(`Correctness : ${avgCorrectness}`, margin + 10, currentY + 26);
    ...
    if (finalScore >= 8) {
      ...
      advice = "Good foundation shown. Imporve clarity and structure. Practice delivering concise, confident answer with stronger supporting exaples."
    }
    else {
        ...
        <div className='md:mb-10 w-full flex items-start gap-4 flex-wrap'>
          ...
          <div>
            ...
            <p className='text-gray-500 mt-2'>
              AI-powerd performance insights
            </p>
          </div>
        </div>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8'>
        ...
        <div className='space-y-6'>
          <motion.div
            ...
            <div className='relative w-20 h-20 sm:h-28 sm:w-28 mx-auto'>
                ...
                <AreaChart data={questionsScoreData}>
                  ...
                  <YAxis
                    domain={[
                      ...
                      Math.max(
                        10,
                        ...questionWiseScore.map((q) => q.score || 0)
                      ),
                    ]}
                  />
                  <Tooltip />
